...
var Pin = function(options) {
  ...
  this.updateCustomer = function(token,fields,callback){
    var req = request.put({url: options.url + '/1/customers/' + token, body: fields, json: true}, callback);
    generateRequest(req);
  };

  this.deleteCustomer = function(token, callback){
    var req = request.delete({url: options.url + '/1/customers/' + token, json: true}, callback);
    generateRequest(req);	
  };
